    ...
    <section className="py-16 md:py-20 bg-gradient-to-br from-cream to-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-cormorant text-mocha mb-6 text-shadow-soft">
            Todo lo que Necesitas Saber
          </h2>
          
          <div className="w-24 h-px bg-sage/40 mx-auto mb-6"></div>
          
          <p className="text-base md:text-lg text-mocha/70 font-karla font-light leading-relaxed">
        ...
        <div className="mb-8 space-y-4">
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-3 pl-10 rounded-xl bg-white/70 backdrop-blur-sm border border-sage/20 focus:border-dusty-rose/30 focus:ring-1 focus:ring-dusty-rose/20 font-karla text-mocha/80 placeholder-mocha/40 shadow-gentle transition-all duration-300 focus:shadow-soft"
            />
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {categories.map(category => (
              <button
                ...
                className={`px-3 py-1.5 text-xs md:text-sm rounded-full font-karla transition-all duration-300 ${activeCategory === category.id ? 'bg-dusty-rose text-white shadow-gentle' : 'bg-white/50 text-mocha/70 hover:bg-white hover:text-dusty-rose'}`}
              >
                {category.label}
              </button>
            ))}
        ...
        <div className="space-y-4">
          {filteredFAQItems.length > 0 ? (
            filteredFAQItems.map((item, index) => (
              ...
              >
                <button
                  ...
                  className="w-full px-4 md:px-6 py-3 md:py-4 text-left flex items-center justify-between hover:bg-sage/5 transition-colors duration-200"
                  aria-expanded={openIndex === index}
                  aria-controls={`faq-answer-${index}`}
                >
                  <h3 className="text-base md:text-lg font-cormorant text-mocha font-medium pr-4">
                    {item.question}
                  </h3>
                  <motion.div
                  ...
                  {openIndex === index && (
                    ...
                    >
                      <div className="px-4 md:px-6 pb-4">
                        <div className="w-full h-px bg-sage/10 my-3 md:mb-4"></div>
                        <p className="text-sm md:text-base text-mocha/80 font-karla font-light leading-relaxed">
                          {item.answer}
                        </p>
                      </div>
                    </motion.div>
            ...
            >
              <p className="text-mocha/60 font-karla italic">No se encontraron preguntas que coincidan con tu búsqueda.</p>
              <button 
                onClick={() => {
                  setSearchQuery('');
        ...
        >
          ...
          <button 
            className="mt-4 px-6 py-2 bg-dusty-rose/90 hover:bg-dusty-rose text-white rounded-full text-sm font-karla shadow-gentle hover:shadow-soft transition-all duration-300 transform hover:scale-105"
