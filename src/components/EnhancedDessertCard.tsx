...
interface DessertCardProps {
  ...
  tags?: string[];
  popularity?: 'trending' | 'bestseller' | 'seasonal' | null;
  onClick?: () => void;
}
...
const EnhancedDessertCard: React.FC<DessertCardProps> = ({
  ...
  const popularityConfig = {
    trending: { label: '🔥 Tendencia', color: 'bg-orange-500' },
    bestseller: { label: '⭐ Más Vendido', color: 'bg-yellow-500' },
    seasonal: { label: '🌸 Temporada', color: 'bg-green-500' }
  };
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        whileHover={{
          boxShadow: "0 25px 50px -12px rgba(131, 106, 93, 0.25), 0 0 0 1px rgba(212, 165, 165, 0.1)"
        }}
      ...
      >
        ...
        {popularity && (
          <motion.div
            className={`absolute top-4 left-4 z-20 ${popularityConfig[popularity].color} text-white text-xs px-3 py-1 rounded-full font-medium shadow-lg`}
        ...
        <div className="p-6 relative">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center space-x-1">
              {renderStars(rating)}
              <span className="text-xs text-mocha/60 ml-1">({rating}.0)</span>
            </div>
