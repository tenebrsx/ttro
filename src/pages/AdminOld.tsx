...
const Admin: React.FC<AdminPanelProps> = () => {
  ...
  // Handle image upload
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, field: 'thumbnailImage' | 'images', index?: number) => {
  ...
  const handleImageUrlChange = (value: string, field: 'thumbnailImage' | 'images', index?: number) => {
    if (field === 'thumbnailImage') {
      setFormData((prev) => ({
        ...prev,
                          ...
                          <div className="flex items-center space-x-4">
                            <div className="flex-1">
                              <input
                                ...
                                onChange={(e) => handleImageUrlChange(e.target.value, 'thumbnailImage')}
                                className="w-full px-4 py-3 rounded-lg border border-dusty-rose/20 focus:border-dusty-rose focus:ring-2 focus:ring-dusty-rose/20 outline-none"
                            ...
                            <div key={index} className="flex items-center space-x-4">
                              <div className="flex-1">
                                <input
                                  type="text"
                                  value={image}
                                  onChange={(e) => handleImageUrlChange(e.target.value, 'images', index)}
                                  className="w-full px-4 py-3 rounded-lg border border-dusty-rose/20 focus:border-dusty-rose focus:ring-2 focus:ring-dusty-rose/20 outline-none"
                        ...
                        {formData.images.some(img => img) && (
                          <div className="grid grid-cols-4 gap-4 mt-4">
                            {formData.images.filter(img => img).map((image, index) => (
                              <div key={index} className="w-20 h-20 rounded-lg overflow-hidden border border-dusty-rose/20">
                                <img
                                  src={image}
                                  alt={`Imagen ${index + 1}`}
                                  className="w-full h-full object-cover"
